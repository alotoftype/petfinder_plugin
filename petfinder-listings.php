...
function petf_shelter_list( $atts ) {
    ...
    echo "http://api.petfinder.com/shelter.getPets?format=json&key=" . trim($petf_options["apikey"]) . "&count=" . trim(intval($count)) . "&id=" . trim($shelter_id) . "&status=" . trim($status) . "&output=full&callback=?";
